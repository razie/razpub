...
import com.razie.pub.hframe.FileUtils;
...
public class AssetLocation implements Serializable {
    ...
    /**
     * make an http URL, if remote. Normally the remote reference is in Weblogic's t3:// format.
     * This will convert it to http://
     */
    ...
    public URL toURL(String fileName) {
        ...
        try {
            ...
            throw new IllegalStateException("can't turn into URL, NewAppEnv=" + this.toString()
                    + " fileName=" + fileName, e);
        }
        return url;
    }
    ...
    public String getHost() {
        if (this.remoteUrl != null && this.remoteUrl.contains("://")) {
            String s = this.remoteUrl.split("://")[1];
            if (s.contains(":")) {
                return s.split(":")[0];
            }
            return s;
    ...
    public String getPort() {
        if (this.remoteUrl != null && this.remoteUrl.contains("::")) {
            String s = this.remoteUrl.split("::")[0];
            if (s.contains(":")) {
                return s.split(":")[2];
            }
            return s;

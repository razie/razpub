...
import com.razie.pub.hframe.UnknownRtException;
...
/**
 * XML utilities to parse/use XML documents - the basic idea is using xpath most of the time to
 * access the contents of the xml doc
 * 
 * $
 * @author razvanc99
 * 
...
public class RiXmlUtils {
     ...
     * 
     * @param url the url. If you want a file use (new File(pathToUse + fname)).toURL()
     */
    public static Document readXml(InputStream is, String id) throws UnknownRtException {
        Document doc = null;
        try {
            ...
            throw new UnknownRtException("Can't read XML file: " + id, mue);
        } catch (java.io.IOException ioe) {
            throw new UnknownRtException("Can't read XML file: " + id, ioe);
        } catch (SAXParseException spe) {
            throw new UnknownRtException("Error parsing XML file: " + id + "line: " + spe.getLineNumber()
                    + ", uri: " + spe.getSystemId(), spe);
        } catch (SAXException se) {
            throw new UnknownRtException("Error parsing XML file: " + id, se);
        } catch (Exception pce) {
            throw new UnknownRtException("Error parsing XML file: " + id, pce);
        }
    }
    ...
    private static final class MyPrefixResolver implements PrefixResolver {
        ...
        public String getNamespaceForPrefix(String prefix, @SuppressWarnings("unused")
        Node context) {
    ...
    public static Element getNode(Node node, String xPath, Map<String, String> prefixes) {
        ...
        if (nl != null) {
            if (nl.size() > 1)
                throw new UnknownRtException("More than 1 node found for xPath " + xPath);
            if (nl.size() == 1)
                return nl.get(0);
        }
        return null;

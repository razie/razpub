...
//        }
   ...
   override def toXml() : String = 
      (for (a <- this.sgetPopulatedAttr) yield "<" + a + ">" + this.getAttr(a) + "</" + a + ">").mkString("")

   override def addToUrl(url:String) : String = {
      val s = (for (a <- sgetPopulatedAttr) yield HttpUtils.toUrlEncodedString(a) + "=" + HttpUtils.toUrlEncodedString(toStr(getAttr(a)))).mkString("&")

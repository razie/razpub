...
public abstract class DrawStream implements DrawAccumulator {
    ...
    public void open(Object d) {
        ...
        if (!recursiveFromContainer) {
            elements.add(new Element(d, Element.State.OPEN));
    ...
    private boolean canWrite(Object d) {
        ...
        int i = index - 1;
        for (i = index - 1; i > 0; i--) {
            if (!this.elements.get(i).state.equals(Element.State.CLOSED))
                break;
        }
    ...
    public void close(Object d) {
        ...
        if (this.elements.get(index).state == Element.State.CLOSED) {
            // already closed - don't recurse
            return;
        }

        this.elements.get(index).state = Element.State.CLOSED;
        ...
        for (int paintingIdx = index; paintingIdx < this.elements.size(); paintingIdx++) {
            Element e = this.elements.get(paintingIdx);
            if (!e.state.equals(Element.State.CLOSED)) {
                break;
            }

            if (d instanceof StreamableContainer) {
                if (((StreamableContainer) d).getState().equals(Element.State.OPEN)) {
                    StreamableContainer.Impl container = (StreamableContainer.Impl) d;
                    container.close();

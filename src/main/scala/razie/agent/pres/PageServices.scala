...
class PageServices extends DrawableSource {
   ...
   private def drawLeft() = {
      ...
      for (val s <- scala.collection.JavaConversions.asBuffer(razie.Agent.instance().copyOfServices()).toList.sort(_.getClass().getSimpleName()<_.getClass().getSimpleName())) {
